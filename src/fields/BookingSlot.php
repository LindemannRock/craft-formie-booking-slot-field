...
class BookingSlot extends FormField implements FormFieldInterface
{
     ...
     * @inheritdoc
     */
    public function getExportLabel(ElementInterface $element, string $exportKey): string
    {
        $label = parent::getExportLabel($element, $exportKey);
    ...
    /**
     * Define export columns - split into Date and Time columns
     */
    public function defineExportColumns(): array
    {
        return [
            'date' => [
                'label' => $this->name . ' - Date',
                'callback' => function($value) {
                    ...
                    // Find and return formatted date label
                    foreach ($availableDates as $dateConfig) {
                        if (($dateConfig['date'] ?? $dateConfig) === $date) {
                            return $dateConfig['label'] ?? $date;
                        }
                    }
            ...
            'slot' => [
                ...
                'callback' => function($value) {
                    ...
                    // Find and return formatted slot label
                    foreach ($timeSlots as $slotConfig) {
                        $slotKey = $slotConfig['startTime'] . '-' . $slotConfig['endTime'];
                        if ($slotKey === $slot) {
