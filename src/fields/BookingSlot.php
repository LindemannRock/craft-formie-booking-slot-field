...
class BookingSlot extends Field implements FieldInterface
{
    ...
    public function getFieldDefaults(): array
    {
        ...
        if ($plugin) {
            $settings = $plugin->getSettings();
            if ($settings) {
                $defaults['operatingHoursStart'] = $settings->defaultOperatingHoursStart ?? '09:00';
                $defaults['operatingHoursEnd'] = $settings->defaultOperatingHoursEnd ?? '17:00';
                $defaults['slotDuration'] = $settings->defaultSlotDuration ?? 60;
                $defaults['maxCapacityPerSlot'] = $settings->defaultMaxCapacityPerSlot ?? 10;
                $defaults['showRemainingCapacity'] = $settings->defaultShowRemainingCapacity ?? true;
                $defaults['dateDisplayType'] = $settings->defaultDateDisplayType ?? 'radio';
                $defaults['slotDisplayType'] = $settings->defaultSlotDisplayType ?? 'radio';
                $defaults['dateSelectionLabel'] = $settings->defaultDateSelectionLabel ?? 'Select Date';
                $defaults['slotSelectionLabel'] = $settings->defaultSlotSelectionLabel ?? 'Select Time Slot';
                $defaults['datePlaceholder'] = $settings->defaultDatePlaceholder ?? 'Select a date...';
                $defaults['slotPlaceholder'] = $settings->defaultSlotPlaceholder ?? 'Select a time slot...';
                $defaults['capacityTemplate'] = $settings->defaultCapacityTemplate ?? '{count} spot(s) left';
                $defaults['fullyBookedText'] = $settings->defaultFullyBookedText ?? 'Fully Booked';
                $defaults['dateDisplayFormat'] = $settings->defaultDateDisplayFormat ?? 'F jS, Y';
                $defaults['timeDisplayFormat'] = $settings->defaultTimeDisplayFormat ?? 'g:i A';
            }
        }
